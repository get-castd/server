...
// cm.ui.settings — Settings wizard component logic
...
const config = {
    cookieName: dialog.dataset.cookieName || "cm_prefs",
...
if (typeof cm !== "undefined" && cm.i18n?.cache) {
    cm.i18n.cache(`#${dialog.id} > .as-wizard > section`);
}
...
const loadPrefsIntoDialog = (prefs) => {
    ...
    // Font size
    const fsRadio = dialog.querySelector(`[name="fontsize"][value="${prefs?.fontSize || "md"}"]`);
...
const savePrefsFromDialog = async () => {
    const newLang = dialog.querySelector('[name="language"]')?.value || document.documentElement.lang || "en-GB";
    const prefs = {
        cookies: {
            essential: true,
            analytics: dialog.querySelector('[name="cookies-analytics"]')?.checked || false,
            marketing: dialog.querySelector('[name="cookies-marketing"]')?.checked || false
        },
        theme: dialog.querySelector('[name="theme-dark"]')?.checked ? "dark" : "light",
    ...
    if (typeof cm !== "undefined" && cm.i18n?.switchLang) {
        localStorage.setItem("cm-lang", newLang);
        cm.i18n.switchLang(newLang);
    }

    // Dispatch custom event for external listeners
    dialog.dispatchEvent(new CustomEvent("cm:settings-saved", {
        bubbles: true,
        detail: { prefs }
    }));
...
const initLiveOptions = () => {
    const darkToggle = dialog.querySelector('[name="theme-dark"]');
    if (darkToggle && !darkToggle._cmInit) {
        darkToggle._cmInit = true;
        darkToggle.onchange = () => {
    ...
    dialog.querySelectorAll('[name="fontsize"]').forEach(radio => {
        if (radio._cmInit) return;
        radio._cmInit = true;
        radio.onchange = () => {
            document.documentElement.style.fontSize = config.fontSizes[radio.value] || "16px";
            // Refresh wizard step to recalculate height
            requestAnimationFrame(() => {
                const wizard = cm?.wizard?.get(`#${wizardEl.id}`);
                if (wizard) {
...
document.addEventListener("cm:wizard-save", (e) => {
...
if (savedPrefs) {
    ...
    if (savedPrefs.lang) {
        ...
        if (typeof cm !== "undefined" && cm.i18n?.lang) {
            cm.i18n.lang(savedPrefs.lang);
        }
    }
}
...
window.cmSettings[dialog.id] = dialog.cmSettings;
