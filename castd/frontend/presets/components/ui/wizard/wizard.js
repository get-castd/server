...
// cn.ui.wizard — Multi-step wizard logic
...
(function () {
  ...
  const init = (el) => {
    ...
    let currentStep = 0;
    const continueText = el.dataset.cnContinue || "Continue";
    const saveText = el.dataset.cnSave || "Save";
    ...
    actionBtn.onclick = () => {
      if (currentStep < steps.length - 1) {
        ...
        el.dispatchEvent(new CustomEvent("cn:wizard-save", { bubbles: true }));
      }
    };
    ...
    const goToStep = (index) => {
      ...
      actionBtn.textContent =
        index === steps.length - 1 ? saveText : continueText;
